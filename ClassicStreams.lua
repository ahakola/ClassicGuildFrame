...
function ClassicStreamDropDownMenu_GetStreamName(clubId, stream)
	local streamName = "";
...
function ClassicStreamDropDownMenu_Initialize(self)
	...
	if self:GetClassicFrame():GetPrivilegesForClub(clubId).canCreateStream and #streams < 50 then
...
function ClassicStreamDropDownMixin:UpdateUnreadNotification()
	local clubId = self:GetClassicFrame():GetSelectedClubId();
	if clubId then
		local ignoreStreamId = self:GetClassicFrame():GetSelectedStreamId();
...
function ClassicStreamDropDownMixin:GetClassicFrame()
	return self:GetParent();
end

ClassicEditStreamDialogMixin = {}

function ClassicEditStreamDialogMixin:OnLoad()
...
function ClassicEditStreamDialogMixin:OnShow()
	local communitiesFrame = self:GetClassicFrame();
	communitiesFrame:RegisterDialogShown(self);
end

function ClassicEditStreamDialogMixin:ShowCreateDialog(clubId)
	self:SetWidth(350);
...
function ClassicEditStreamDialogMixin:ShowEditDialog(clubId, stream)
	self:SetWidth(400);
...
function ClassicEditStreamDialogMixin:UpdateAcceptButton()
	self.Accept:SetEnabled(self.NameEdit:GetText() ~= "");
end

function ClassicEditStreamDialogMixin:GetClassicFrame()
	return self:GetParent();
end

ClassicNotificationSettingsStreamEntryMixin = {};

function ClassicNotificationSettingsStreamEntryMixin:SetStream(clubId, streamId)
	self.clubId = clubId;
	self.streamId = streamId;
end

function ClassicNotificationSettingsStreamEntryMixin:GetStreamId()
	return self.streamId;
end

function ClassicNotificationSettingsStreamEntryMixin:SetFilter(filter)
...
function ClassicNotificationSettingsStreamEntryMixin:GetFilter()
	return self.filter;
end

ClassicNotificationSettingsDialogMixin = {};

function ClassicNotificationSettingsDialogMixin:OnLoad()
	self.buttonPool = CreateFramePool("BUTTON", self.ScrollFrame.Child, "ClassicNotificationSettingsStreamEntryTemplate");
end

function ClassicNotificationSettingsDialogMixin:OnShow()
	self:GetClassicFrame():RegisterDialogShown(self);
end

function ClassicNotificationSettingsDialogMixin:Refresh()
...
function ClassicNotificationSettingsDialogMixin:SaveSettings()
	local clubId = self:GetSelectedClubId();
	if clubId then
...
function ClassicNotificationSettingsDialogMixin:SetAll(filter)
	for button in self.buttonPool:EnumerateActive() do
		button:SetFilter(filter);
	end
end

function ClassicNotificationSettingsDialogMixin:Cancel()
	self:Hide();
end

function ClassicNotificationSettingsDialogMixin:SelectClub(clubId)
	self.clubId = clubId;
	...
	self.ClassicListDropDownMenu:OnClubSelected();
	self:Refresh();
end

function ClassicNotificationSettingsDialogMixin:GetSelectedClubId()
	return self.clubId;
end

function ClassicNotificationSettingsDialogMixin:GetClassicFrame()
	return self:GetParent();
end

function ClassicMassNotificationsSettingsButton_OnClick(self)
	PlaySound(SOUNDKIT.IG_MAINMENU_OPTION_CHECKBOX_ON);
	self:GetParent():GetParent():GetParent():SetAll(self.filter);
end

function ClassicNotificationSettingsDialogOkayButton_OnClick(self)
	self:GetParent():SaveSettings();
	self:GetParent():Hide();
end

function ClassicNotificationSettingsDialogCancelButton_OnClick(self)
	self:GetParent():Cancel();
end

--[[
ClassicAddToChatMixin = {};

function ClassicAddToChatMixin:OnLoad()
	SquareButton_SetIcon(self, "DOWN");
end

function ClassicAddToChatMixin:SetClubId(clubId)
	self.clubId = clubId;
end

function ClassicAddToChatMixin:GetClubId()
	return self.clubId;
end

function ClassicAddToChatMixin:SetStreamId(streamId)
	self.streamId = streamId;
end

function ClassicAddToChatMixin:GetStreamId()
	return self.streamId;
end

function ClassicAddToChatMixin:OnClick()
	PlaySound(SOUNDKIT.IG_MAINMENU_OPTION_CHECKBOX_ON);
...
function ClassicAddToChatDropDown_Initialize(self, level)
...
function ClassicAddStreamHighlightTab_OnClick(self)
