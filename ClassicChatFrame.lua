...
function ClassicChatMixin:OnLoad()
...
function ClassicChatMixin:OnShow()
...
function ClassicChatMixin:OnEvent(event, ...)
	if event == "CLUB_MESSAGE_ADDED" then
		local clubId, streamId, messageId = ...;
		if clubId == self:GetClassicFrame():GetSelectedClubId() and streamId == self:GetClassicFrame():GetSelectedStreamId() then
	...
	elseif event == "CLUB_UPDATED" then
		local clubId = ...;
		if clubId == self:GetClassicFrame():GetSelectedClubId() then
...
function ClassicChatMixin:OnHide()
	FrameUtil.UnregisterFrameForEvents(self, COMMUNITIES_CHAT_FRAME_EVENTS);
	if self.streamSelectedCallback then
		self:GetClassicFrame():UnregisterCallback(ClassicFrameMixin.Event.StreamSelected, self.streamSelectedCallback);
		self.streamSelectedCallback = nil;
	end
end

function ClassicChatMixin:SendMessage(text)
...
function ClassicChatMixin:GetMessagesToDisplay()
	local clubId = self:GetClassicFrame():GetSelectedClubId();
	local streamId = self:GetClassicFrame():GetSelectedStreamId();
...
function ClassicChatMixin:HasAllMessages(clubId, streamId)
...
function ClassicChatMixin:RequestInitialMessages(clubId, streamId)
	local ranges = C_Club.GetMessageRanges(clubId, streamId);
...
function ClassicChatMixin:RequestMoreHistory()
	local communitiesFrame = self:GetClassicFrame();
...
function ClassicChatMixin:BackfillMessages(maxCount)
	local clubId = self:GetClassicFrame():GetSelectedClubId();
	local streamId = self:GetClassicFrame():GetSelectedStreamId();
	if not clubId or not streamId then
		return;
...
function ClassicChatMixin:DisplayChat()
...
function ClassicChatMixin:UpdateScrollbar()
...
function ClassicChatMixin:UpdateChatColor()
...
function ClassicChatMixin:GetChatColor()
	local clubId = self:GetClassicFrame():GetSelectedClubId();
	if not clubId then
		return nil;
	end
...
function ClassicChatMixin:FormatMessage(clubId, streamId, message)
...
function ClassicChatMixin:AddDateNotification(date, backfill)
...
function ClassicChatMixin:AddUnreadNotification(backfill)
...
function ClassicChatMixin:AddNotification(notification, atlas, r, g, b, backfill)
...
function ClassicChatMixin:AddBroadcastMessage(clubId)
	local clubInfo = C_Club.GetClubInfo(clubId);
...
function ClassicChatMixin:AddMessage(clubId, streamId, message, backfill)
	local r, g, b = self:GetChatColor();
	if not r then
...
function ClassicChatMixin:RegisterForMemberUpdate(clubId, memberId)
...
function ClassicChatMixin:GetClassicFrame()
	return self:GetParent();
end

function ClassicChatMixin:RefreshMessages(predicate)
...
function ClassicChatEditBox_OnFocusGained(self)
	EditBox_HighlightText(self);
	ChatFrame_SetChatFocusOverride(self);
end

function ClassicChatEditBox_OnEnterPressed(self)
...
function ClassicChatEditBox_OnHide(self)
	if ChatFrame_GetChatFocusOverride() == self then
		ChatFrame_ClearChatFocusOverride();
	end
end

function ClassicChatFrameScrollBar_OnValueChanged(self, value, userInput)
...
function ClassicJumpToUnreadButton_OnClick(self)
end
