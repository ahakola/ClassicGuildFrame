...
function ClassicGuildRewardsFrame_OnLoad(self)
	ClassicGuildFrame_RegisterPanel(self);
	ClassicGuildRewardsContainer.update = ClassicGuildRewards_Update;
	HybridScrollFrame_CreateButtons(ClassicGuildRewardsContainer, "ClassicGuildRewardsButtonTemplate", 1, 0);
	ClassicGuildRewardsContainerScrollBar.doNotHide = true;
	self:RegisterEvent("GUILD_REWARDS_LIST");
end

function ClassicGuildRewardsFrame_OnShow(self)
	RequestGuildRewards();
end
...
function ClassicGuildRewardsFrame_OnEvent(self, event)
	ClassicGuildRewards_Update();
end

function ClassicGuildRewards_Update()
	local scrollFrame = ClassicGuildRewardsContainer;
...
function ClassicGuildRewardsButton_OnEnter(self)
	ClassicGuildRewardsFrame.activeButton = self;
	...
	self.UpdateTooltip = ClassicGuildRewardsButton_OnEnter;
	GameTooltip:Show();
end

function ClassicGuildRewardsButton_OnLeave(self)
	GameTooltip:Hide();
	ClassicGuildRewardsFrame.activeButton = nil;
	self.UpdateTooltip = nil;
end

function ClassicGuildRewardsButton_OnClick(self, button)
	if ( IsModifiedClick("CHATLINK") ) then
		local achievementID, itemID, itemName, iconTexture, repLevel, moneyCost = GetGuildRewardInfo(self.index);
		ClassicGuildFrame_LinkItem(_, itemID);
...
function ClassicGuildRewardsDropDown_OnHide(self)
	ClassicGuildRewardsDropDown.rewardIndex = nil;
end